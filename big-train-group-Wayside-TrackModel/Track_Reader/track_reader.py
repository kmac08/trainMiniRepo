...
class TrackBlock:
    ...
    has_yard_connection: bool = False  # True if block connects to yard (0)
    ...
    def leads_to_yard(self) -> bool:
        ...
        Returns:
            True if block connects to yard (0), False otherwise
        """
...
class TrackLayoutReader:
    ...
    def _normalize_platform_side(self, side_str: str) -> str:
        ...
        if side_upper in ["LEFT", "RIGHT"]:
            return str(side_str).strip()
    ...
    def _parse_connected_blocks(self, connected_str) -> Tuple[List[int], bool]:
        ...
        Maps -1 (yard in Excel) to 0 (yard in system) as requested.
        ...
        if isinstance(connected_str, (int, float)):
            ...
            if block_num == 0:  # Excel uses 0 for yard
                has_yard = True
                self._debug_print(f"    Single connection: 0 (yard)")
            else:
            ...
            for part in parts:
                if part:
                    try:
                        ...
                        if block_num == 0:  # Excel uses 0 for yard
                            has_yard = True
                            self._debug_print(f"    Connection: 0 (yard)")
                        else:
                            connected_blocks.append(block_num)
                    except ValueError:
    ...
    def _load_track_data(self):
        ...
        try:
            # Read the Excel file using context manager to ensure proper closing
            self._debug_print(f"Loading track data from: {self.file_path}")
            with pd.ExcelFile(self.file_path) as excel_data:
            ...
            # Always show station numbering for debugging
            print("\n=== STATION NUMBERING ===")
            for line_name in self.selected_lines:
                stations = self.get_all_stations(line_name)
                if stations:
                    print(f"{line_name} Line stations ({len(stations)} total):")
                    for station in stations:
                        print(f"  - Station {station['station_id']}: Block {station['block_number']}: {station['station_name']} ({station['platform_side']})")
    ...
    def get_line_for_block(self, block_number: int) -> Optional[str]:
        """
        Get the line name for a specific block number.
        
        Args:
            block_number: The block number to find the line for
            
        Returns:
            Line name ('Red', 'Green', 'Blue') if found, None otherwise
        """
        block = self.get_block_by_number(block_number)
        return block.line if block else None
