...
# Add path for wayside controller imports
project_root = os.path.dirname(os.path.dirname(__file__))
sys.path.insert(0, project_root)
sys.path.insert(0, os.path.join(project_root, 'CTC'))
sys.path.insert(0, os.path.join(project_root, 'CTC', 'Core'))
sys.path.insert(0, os.path.join(project_root, 'Track_Reader'))
sys.path.insert(0, os.path.join(project_root, 'Wayside_Controller'))

# ============================================================================
# WAYSIDE CONTROLLER CONFIGURATION - Easy to Modify
# ============================================================================
# Configure wayside controllers for each line here
# Each controller manages a specific range of blocks and uses a PLC file
# Block ranges should not overlap within the same line

WAYSIDE_CONFIG = {
    "Red": [
        {
            "id": "Red_Controller_1",
            "blocks": list(range(0, 39)),  # Yard + Blocks 1-38
            "plc_file": "Wayside_Controller\\RedLinePlcV1.py"  # Use Red Line PLC
        },
        {
            "id": "Red_Controller_2", 
            "blocks": list(range(39, 77)),  # Blocks 39-76
            "plc_file": "Wayside_Controller\\RedLinePlcV1.py"  # Use Red Line PLC
        }
    ],
    "Green": [
        {
            "id": "Green_Controller_1",
            "blocks": list(range(0, 76)),  # Yard + Blocks 1-75
            "plc_file": "Wayside_Controller\\GreenLinePlcV1.py"
        },
        {
            "id": "Green_Controller_2",
            "blocks": list(range(76, 151)),  # Blocks 76-150
            "plc_file": "Wayside_Controller\\GreenLinePlcV1.py"  # Reuse same PLC for now
        }
    ],
    "Blue": [
        {
            "id": "Blue_Controller_1",
            "blocks": list(range(0, 16)),  # Yard + Blocks 1-15 (all Blue line blocks)
            "plc_file": "Wayside_Controller\\GreenLinePlcV1.py"  # Reuse Green PLC for Blue line
        }
    ]
}
...
class TimeManager(QThread):
    ...
    def get_time(self):
        """Get current simulation time as datetime object with microsecond precision"""
        # Parse start time
        try:
            start_hour, start_minute = map(int, self.simulation_start_time.split(':'))
        except:
        ...
        current_microsecond = int((total_seconds % 1) * 1_000_000)  # Extract fractional seconds as microseconds
...
class WaysideManager:
    """Manages wayside controllers for all lines based on configuration"""
    
    def __init__(self, master_interface):
        self.master_interface = master_interface
        self.active_controllers = {}  # line -> list of controllers
        self.track_reader = None
        self.ctc_system = None
        
    def setup_wayside_controllers(self, selected_lines: List[str], ctc_system, track_reader):
        """Set up wayside controllers for selected lines"""
        self.ctc_system = ctc_system
        self.track_reader = track_reader
        print("Setting up wayside controllers for selected lines:")
        self.master_interface.log_status("Setting up wayside controllers...")
        
        for line in selected_lines:
            if line in WAYSIDE_CONFIG:
                self.master_interface.log_status(f"Creating controllers for {line} line...")
                print(f"Creating controllers for {line} line")
                self._setup_controllers_for_line(line)
            else:
                self.master_interface.log_status(f"Warning: No configuration found for {line} line")
                print(f"Warning: No configuration found for {line} line")
                
        self.master_interface.log_status(f"Wayside setup complete - {self.get_total_controller_count()} controllers active")
        
    def _setup_controllers_for_line(self, line: str):
        """Set up all controllers for a specific line"""
        line_controllers = []
        
        for controller_config in WAYSIDE_CONFIG[line]:
            print(f"Creating controller for {line} line and {controller_config['id']}")
            try:
                print(f"Entering try block")
                # Import wayside controller components
                ...
                # Get the actual number of blocks for this line from track reader
                line_summary = self.track_reader.get_line_summary(line)
                total_blocks = line_summary['total_blocks'] + 1  # Include yard (block 0)
                
                # Create blocks_covered boolean array with correct size
                blocks_covered = [False] * total_blocks
                for block in controller_config["blocks"]:
                    if 0 <= block < total_blocks:
                        blocks_covered[block] = True
                        
                # Create data structure for wayside controller
                wayside_data = {
                    line: {
                        "auto": self.track_reader.lines[line]
                    }
                }
                
                # Create absolute path for PLC file
                project_root = os.path.dirname(os.path.dirname(__file__))
                plc_file_path = os.path.join(project_root, controller_config["plc_file"])
                
                # Create wayside controller
                controller = WaysideController(
                    data=wayside_data,
                    line=line,
                    mode="auto",
                    auto=True,
                    plc_num=len(line_controllers) + 1,  # Sequential numbering
                    plc_file=plc_file_path,
                    blocks_covered=blocks_covered,
                    total_blocks=total_blocks
                )
                
                # Set controller ID
                controller.controller_id = controller_config["id"]
                
                # Register with CTC system
                red_line = (line == "Red")
                communication_handler = self.ctc_system.provide_wayside_controller(
                    waysideControllerObject=controller,
                    blocksCovered=blocks_covered,
                    redLine=red_line
                )

                controller.set_communication_object(communication_handler)
                print("over here wayside")
                controller.set_track_model_communication_object(CommunicationObject(controller_config["id"], line))
                
                # Start the update cycle timer
                controller.start_update_cycle()
                
                # Create Track Model CommunicationObject for this wayside
                from Track_Model.trackmodel_working import CommunicationObject
                track_model_comm = CommunicationObject(
                    wayside_id=controller_config["id"],
                    wayside_line=line
                )
                # Store reference for Track Model to read from
                controller.track_model_communication = track_model_comm
                
                line_controllers.append(controller)
                
                self.master_interface.log_status(f"✓ {controller_config['id']}: blocks {min(controller_config['blocks'])}-{max(controller_config['blocks'])}")
                
            except Exception as e:
                self.master_interface.log_status(f"✗ Failed to create {controller_config['id']}: {e}")
                print(f"Failed to create {controller_config['id']}: {e}")
                
        self.active_controllers[line] = line_controllers
        
    def get_total_controller_count(self) -> int:
        """Get total number of active controllers"""
        return sum(len(controllers) for controllers in self.active_controllers.values())
        
    def get_controller_status(self) -> str:
        """Get status string for all controllers"""
        if not self.active_controllers:
            return "No controllers active"
            
        status_parts = []
        for line, controllers in self.active_controllers.items():
            status_parts.append(f"{line}: {len(controllers)} controllers")
            
        return " | ".join(status_parts)
        
    def shutdown_all_controllers(self):
        """Shutdown all active wayside controllers"""
        if not self.active_controllers:
            return
            
        self.master_interface.log_status("Shutting down wayside controllers...")
        
        for line, controllers in self.active_controllers.items():
            for controller in controllers:
                try:
                    controller.stop_update_cycle()
                    self.master_interface.log_status(f"✓ Stopped {controller.controller_id}")
                except Exception as e:
                    self.master_interface.log_status(f"✗ Error stopping {controller.controller_id}: {e}")
                    
        self.active_controllers.clear()
        self.master_interface.log_status("All wayside controllers shutdown")
        
class CommunicationObject:
    """
    Simple Communication interface for Track Model to interact with individual Wayside Controllers.
    Basic setter/getter only - no validation or computation.
    ...
    def __init__(self, wayside_id: str, wayside_line: str = "Green"):
        ...
        block_length = 16
        if wayside_line == "Green":
            block_length = 151
        elif wayside_line == "Red":
            block_length = 77
        # Initialize all data arrays with default values (151 elements: G0-G150)
        self._wayside_blocks_covered = ["0"] * block_length     # 1-bit: determines which blocks this wayside manages
        self._next_block_numbers = ["0000000"] * block_length    # 7-bit: next block numbers
        self._next_station_numbers = ["00000"] * block_length    # 5-bit: next station numbers
        self._update_block_in_queue = ["0"] * block_length       # 1-bit: update block in queue
        self._wayside_authority = ["0"] * block_length           # 1-bit: authority for each block
        self._wayside_commanded_speed = ["00"] * block_length    # 2-bit: commanded speed
        self._switch_states = ["0"] * block_length               # 1-bit: switch states
        self._traffic_light_states = ["0"] * block_length        # 1-bit: traffic light states
        self._crossing_states = ["0"] * block_length             # 1-bit: crossing states
        print(f"CommunicationObject initialized for Wayside {wayside_id} on {wayside_line} line")
    
    # === WAYSIDE BLOCKS COVERED ===
    
    def setWaysideBlocksCovered(self, blocks_covered_list):
        self._wayside_blocks_covered = blocks_covered_list
    
    def getWaysideBlocksCovered(self):
        return self._wayside_blocks_covered
    
    # === AUTHORITY ===
    
    def setAuthorities(self, authorities_list):
        self._wayside_authority = authorities_list
    
    def getWaysideAuthority(self):
        return self._wayside_authority
    
    # === COMMANDED SPEED ===
    
    def setCommandedSpeeds(self, speeds_list):
        self._wayside_commanded_speed = speeds_list
    
    def getWaysideCommandedSpeed(self):
        return self._wayside_commanded_speed
    
    # === NEXT BLOCK NUMBERS ===
    
    def setNextBlockNumbers(self, next_blocks_list):
        self._next_block_numbers = next_blocks_list
    
    def getNextBlockNumbers(self):
        return self._next_block_numbers
    
    # === NEXT STATION NUMBERS ===
    
    def setNextStationNumbers(self, next_stations_list):
        self._next_station_numbers = next_stations_list
    
    def getNextStationNumbers(self):
        return self._next_station_numbers
    
    # === UPDATE BLOCK IN QUEUE ===
    
    def setUpdateBlockInQueue(self, update_queue_list):
        self._update_block_in_queue = update_queue_list
    
    def getUpdateBlockInQueue(self):
        return self._update_block_in_queue
    
    # === SWITCH STATES ===
    
    def setSwitchStates(self, switch_states_list):
        self._switch_states = switch_states_list
    ...
    def setTrafficLightStates(self, traffic_light_list):
        self._traffic_light_states = traffic_light_list
    ...
    def setCrossingStates(self, crossing_states_list):
        self._crossing_states = crossing_states_list
    
    def getCrossingStates(self):
        return self._crossing_states
    
    # === BLOCK OCCUPANCY ===
    
    def getBlockOccupancy(self):
        """Get block occupancy states for track model integration"""
        # Return a list of boolean values indicating block occupancy
        # For now, return all False since this is a placeholder implementation
        # Use the correct array size based on the line
        block_length = 16  # Default for Blue
        if self.wayside_line == "Green":
            block_length = 151
        elif self.wayside_line == "Red":
            block_length = 77
        
        return [False] * block_length
...
class MasterInterface(QMainWindow):
    ...
    def __init__(self):
        ...
        self.wayside_manager = WaysideManager(self)  # Initialize wayside manager
    ...
    def on_time_update(self, current_time_str):
        ...
        # Send time update to Track Model if it's running
        if hasattr(self, 'track_model_interface') and self.track_model_interface is not None:
            self.track_model_interface.update_time(current_time_str)
    ...
    def launch_track_model(self):
        """Launch Track Model module with selected parameters"""
        try:
            # Import Track Model interface
            project_root = os.path.dirname(os.path.dirname(__file__))
            sys.path.insert(0, project_root)
            from Track_Model.trackmodel_working import TrackModelInterface
            
            # Create Track Model interface with selected parameters
            track_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 
                                    "Track_Reader", "Track Layout & Vehicle Data vF2.xlsx")
            
            self.track_model_interface = TrackModelInterface(
                track_file=track_file, 
                selected_lines=self.selected_lines
            )
            
            # Show Track Model interface
            self.track_model_interface.show()
            
            # Set initial time
            current_time = self.time_manager.get_current_time_string()
            self.track_model_interface.update_time(current_time)
            
            # Store in active modules
            self.active_modules["TrackModel"] = self.track_model_interface
            
            self.log_status(f"Track Model launched with lines: {', '.join(self.selected_lines)}")
            
        except Exception as e:
            self.log_status(f"Error launching Track Model: {str(e)}")
            
    def stop_track_model(self):
        """Stop Track Model module"""
        if "TrackModel" in self.active_modules:
            try:
                # Close Track Model interface
                self.active_modules["TrackModel"].close()
                del self.active_modules["TrackModel"]
                self.track_model_interface = None
                
                self.log_status("Track Model stopped")
                
            except Exception as e:
                self.log_status(f"Error stopping Track Model: {str(e)}")
    ...
    def start_system(self):
        ...
        # Setup wayside controllers for selected lines
        print("Setting up CTC-Wayside integration")
        if self.ctc_interface and hasattr(self.ctc_interface, 'ctc_system'):
            try:
                # Import track reader for wayside setup
                from Track_Reader.track_reader import TrackLayoutReader
                
                track_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 
                                        "Track_Reader", "Track Layout & Vehicle Data vF2.xlsx")
                track_reader = TrackLayoutReader(track_file)
                print("Track reader loaded.")
                
                # Setup wayside controllers
                self.wayside_manager.setup_wayside_controllers(
                    selected_lines=self.selected_lines,
                    ctc_system=self.ctc_interface.ctc_system,
                    track_reader=track_reader
                )
                print("Wayside controllers setup complete")
                self.log_status(f"Wayside controllers active: {self.wayside_manager.get_controller_status()}")
                
            except Exception as e:
                self.log_status(f"Warning: Wayside controller setup failed: {e}")
            
        self.log_status("Complete system started")
            
    def stop_all_modules(self):
        """Stop all running modules"""
        # Stop wayside controllers first
        try:
            self.wayside_manager.shutdown_all_controllers()
        except Exception as e:
            self.log_status(f"Error stopping wayside controllers: {e}")
        ...
        for module_name, module in list(self.active_modules.items()):
            try:
                if module_name == "CTC" and hasattr(module, 'close'):
                    ...
                    module.close()
                    self.ctc_interface = None
                elif module_name == "TrackModel" and hasattr(module, 'close'):
                    # Handle Track Model interface directly
                    module.close()
                    self.track_model_interface = None
                else:
