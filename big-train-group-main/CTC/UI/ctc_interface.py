...
class CTCInterface(QMainWindow):
	...
	def __init__(self, track_file: str = "Track Layout & Vehicle Data vF2.xlsx", selected_lines: list = None, time_multiplier: float = 1.0):
		...
		if len(selected_lines) == 1:
			...
			self.selectedLine = selected_lines[0]
			self.selectedDisplayLine = selected_lines[0]
		self.shiftStartTime = datetime.now().replace(hour=6, minute=0, second=0)
		...
		self.updateWorker.updateData.connect(self.update_data_only)
...
# Removed auto-resize functionality that makes map too small
		# QTimer.singleShot(300, self.resize_panels_for_tables)
		# QTimer.singleShot(400, self.update_scheduled_closures_display)  # Temporarily disabled
		
		self.shown_conflict_ids = set()  # Track shown emergency conflict popups
	...
	def setup_main_interface(self):
		...
		# Throughput info on left - made larger
		self.throughput_label = QLabel("Green Line: 20 pass/hr  |  Red Line: 30 pass/hr")
		...
		self.setup_resolve_emergency_tab()
	...
	def setup_main_tab(self):
		...
		self.resolve_btn = QPushButton("Resolve Emergency")
		self.resolve_btn.setMaximumHeight(40)
		self.resolve_btn.clicked.connect(self.start_emergency_resolution)
		warnings_layout.addWidget(self.resolve_btn)
	...
	def populate_initial_dropdowns(self):
		...
		try:
			...
			print(f"[DEBUG] populate_initial_dropdowns called for line: {line}")
	...
	def check_block_close_possibility(self, line, block_num, close_time):
		"""Dummy function to check if block can be closed"""
		# For demo purposes, randomly return false sometimes
		import random
		if random.random() < 0.3:  # 30% chance of conflict
			return False, "05:15"  # Return earliest possible time
		return True, None
		...
		if is_open_mode:
			...
			try:
				...
				if block:
					...
					# Check if block is currently closed
					is_closed = block in self.failure_manager.failedBlocks
	...
	def update_close_block_table_display(self):
		...
		for row in range(self.close_block_table.rowCount()):
			...
			if line_item and block_item:
				...
				# Check if block is failed/closed using failure manager
				block = self.ctc_system.get_block_by_line(line, block_num)
				is_closed = False
				if block and hasattr(self.failure_manager, 'failedBlocks'):
					is_closed = block in self.failure_manager.failedBlocks
				open_text = "○" if is_closed else "●"  # ● = filled circle (open), ○ = empty circle (closed)
				if open_item:
					open_item.setText(open_text)

	def setup_resolve_emergency_tab(self):
		"""Setup the resolve emergency tab to match classDiagram.png layout"""
		resolve_widget = QWidget()
		resolve_layout = QVBoxLayout()
		resolve_widget.setLayout(resolve_layout)

		# Train in Error section
		error_frame = QFrame()
		error_frame.setFrameStyle(QFrame.Box)
		error_layout = QVBoxLayout()
		error_frame.setLayout(error_layout)

		error_label = QLabel("Train in Error")
		error_label.setFont(QFont("Arial", 18, QFont.Bold))  # Header size per style guide
		error_layout.addWidget(error_label)

		self.train_error_table = QTableWidget()
		self.train_error_table.setColumnCount(8)
		self.train_error_table.setHorizontalHeaderLabels(['Error Type', 'Train ID', 'Line', 'Location (Section)', 'Location (Block)', 'Destination (Section)', 'Destination (Block)', 'ETA', 'Speed (mph)'])
		self.train_error_table.setMaximumHeight(80)
		self.train_error_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
		self.train_error_table.horizontalHeader().setStretchLastSection(False)
		# Set proper column widths for 20pt font
		self.train_error_table.setColumnWidth(0, 130)  # Error Type
		self.train_error_table.setColumnWidth(1, 120)  # Train ID
		self.train_error_table.setColumnWidth(2, 100)  # Line
		self.train_error_table.setColumnWidth(3, 150)  # Location (Section)
		self.train_error_table.setColumnWidth(4, 130)  # Location (Block)
		self.train_error_table.setColumnWidth(5, 160)  # Destination (Section)
		self.train_error_table.setColumnWidth(6, 140)  # Destination (Block)
		self.train_error_table.setColumnWidth(7, 120)  # ETA
		self.train_error_table.verticalHeader().setVisible(False)
		self.train_error_table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # Make table non-editable
		# Set fonts according to style guide: 20pt content for better readability
		self.train_error_table.setFont(QFont("Arial", 20))
		self.train_error_table.horizontalHeader().setFont(QFont("Arial", 16, QFont.Bold))
		error_layout.addWidget(self.train_error_table)

		resolve_layout.addWidget(error_frame)

		# Trains with Affected Routes section
		affected_frame = QFrame()
		affected_frame.setFrameStyle(QFrame.Box)
		affected_layout = QVBoxLayout()
		affected_frame.setLayout(affected_layout)

		affected_label = QLabel("Trains with Effected Routes")
		affected_label.setFont(QFont("Arial", 18, QFont.Bold))  # Header size per style guide
		affected_layout.addWidget(affected_label)

		self.affected_trains_table = QTableWidget()
		self.affected_trains_table.setColumnCount(8)
		self.affected_trains_table.setHorizontalHeaderLabels(['Train ID', 'Line', 'Location (Section)', 'Location (Block)', 'Destination (Section)', 'Destination (Block)', 'ETA', 'Speed (mph)'])
		self.affected_trains_table.setMaximumHeight(120)
		self.affected_trains_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
		self.affected_trains_table.horizontalHeader().setStretchLastSection(False)
		# Set proper column widths for 20pt font
		self.affected_trains_table.setColumnWidth(0, 120)  # Train ID
		self.affected_trains_table.setColumnWidth(1, 100)  # Line
		self.affected_trains_table.setColumnWidth(2, 150)  # Location (Section)
		self.affected_trains_table.setColumnWidth(3, 130)  # Location (Block)
		self.affected_trains_table.setColumnWidth(4, 160)  # Destination (Section)
		self.affected_trains_table.setColumnWidth(5, 140)  # Destination (Block)
		self.affected_trains_table.setColumnWidth(6, 120)  # ETA
		self.affected_trains_table.setColumnWidth(7, 130)  # Speed (mph)
		self.affected_trains_table.verticalHeader().setVisible(False)
		self.affected_trains_table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # Make table non-editable
		# Set fonts according to style guide: 20pt content for better readability
		self.affected_trains_table.setFont(QFont("Arial", 20))
		self.affected_trains_table.horizontalHeader().setFont(QFont("Arial", 16, QFont.Bold))
		affected_layout.addWidget(self.affected_trains_table)

		# Action buttons for affected trains
		affected_buttons_layout = QHBoxLayout()
		self.auto_reroute_btn = QPushButton("Automatically Re-Route All Effected Trains")
		self.stop_all_btn = QPushButton("Stop All Effected Trains")
		self.auto_reroute_btn.clicked.connect(self.auto_reroute_all_trains)
		self.stop_all_btn.clicked.connect(self.stop_all_affected_trains)
		affected_buttons_layout.addWidget(self.auto_reroute_btn)
		affected_buttons_layout.addWidget(self.stop_all_btn)
		affected_layout.addLayout(affected_buttons_layout)

		resolve_layout.addWidget(affected_frame)

		# Successfully Rerouted Trains section
		success_frame = QFrame()
		success_frame.setFrameStyle(QFrame.Box)
		success_layout = QVBoxLayout()
		success_frame.setLayout(success_layout)

		success_label = QLabel("Successfully Rerouted Trains")
		success_label.setFont(QFont("Arial", 18, QFont.Bold))  # Header size per style guide
		success_layout.addWidget(success_label)

		self.success_trains_table = QTableWidget()
		self.success_trains_table.setColumnCount(8)
		self.success_trains_table.setHorizontalHeaderLabels(['Train ID', 'Line', 'Location (Section)', 'Location (Block)', 'Destination (Section)', 'Destination (Block)', 'ETA', 'Speed (mph)'])
		self.success_trains_table.setMaximumHeight(80)
		self.success_trains_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
		self.success_trains_table.verticalHeader().setVisible(False)
		self.success_trains_table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # Make table non-editable
		# Set larger font for table
		self.success_trains_table.setFont(QFont("Arial", 12))
		self.success_trains_table.horizontalHeader().setFont(QFont("Arial", 18, QFont.Bold))
		success_layout.addWidget(self.success_trains_table)

		resolve_layout.addWidget(success_frame)

		# Trains That Were Unable to be Rerouted section
		unable_frame = QFrame()
		unable_frame.setFrameStyle(QFrame.Box)
		unable_layout = QVBoxLayout()
		unable_frame.setLayout(unable_layout)

		unable_label = QLabel("Trains That Were Unable to be Rerouted")
		unable_label.setFont(QFont("Arial", 18, QFont.Bold))  # Header size per style guide
		unable_layout.addWidget(unable_label)

		instruction_label = QLabel("Select each train and either manually re-route it using the route train interface or just halt the train.")
		instruction_label.setFont(QFont("Arial", 14))  # Default size per style guide
		unable_layout.addWidget(instruction_label)

		self.unable_trains_table = QTableWidget()
		self.unable_trains_table.setColumnCount(8)
		self.unable_trains_table.setHorizontalHeaderLabels(['Train ID', 'Line', 'Location (Section)', 'Location (Block)', 'Destination (Section)', 'Destination (Block)', 'ETA', 'Speed (mph)'])
		self.unable_trains_table.setMaximumHeight(120)
		self.unable_trains_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
		self.unable_trains_table.verticalHeader().setVisible(False)
		self.unable_trains_table.setSelectionBehavior(QAbstractItemView.SelectRows)
		self.unable_trains_table.setEditTriggers(QAbstractItemView.NoEditTriggers)  # Make table non-editable
		# Set larger font for table
		self.unable_trains_table.setFont(QFont("Arial", 12))
		self.unable_trains_table.horizontalHeader().setFont(QFont("Arial", 18, QFont.Bold))
		unable_layout.addWidget(self.unable_trains_table)

		# Manual action buttons
		manual_buttons_layout = QHBoxLayout()
		self.manual_reroute_btn = QPushButton("Manually Re-Route Train")
		self.stop_train_btn = QPushButton("Stop This Train")
		self.manual_reroute_btn.clicked.connect(self.manually_reroute_selected_train)
		self.stop_train_btn.clicked.connect(self.stop_selected_train)
		manual_buttons_layout.addWidget(self.manual_reroute_btn)
		manual_buttons_layout.addWidget(self.stop_train_btn)
		unable_layout.addLayout(manual_buttons_layout)

		resolve_layout.addWidget(unable_frame)

		# Finish button
		finish_layout = QHBoxLayout()
		finish_layout.addStretch()
		self.finish_emergency_btn = QPushButton("Finish Resolving Emergency")
		self.finish_emergency_btn.clicked.connect(self.finish_resolving_emergency)
		finish_layout.addWidget(self.finish_emergency_btn)
		finish_layout.addStretch()
		resolve_layout.addLayout(finish_layout)

		resolve_layout.addStretch()

		# Initialize emergency state
		self.emergency_context = None
		self.successfully_rerouted_trains = []
		self.unable_to_reroute_trains = []

		self.tab_widget.addTab(resolve_widget, "Resolve Emergency")

	# Emergency handling functions
	def start_emergency_resolution(self):
		"""Start emergency resolution with sample data or detected emergency"""
		# For demo purposes, create a sample emergency
		# In a real system, this would detect the actual emergency from system state
		
		# Create some sample trains if none exist
		# Check if trains exist in the new CTC system
		if not self.ctc_system.trains:
			# Create mock train objects for testing
			try:
				mock_train_1 = type('MockTrain', (), {'trainID': '1234', 'currentBlock': 17, 'route': None})()
				mock_train_2 = type('MockTrain', (), {'trainID': '5678', 'currentBlock': 9, 'route': None})()
				mock_train_3 = type('MockTrain', (), {'trainID': '2468', 'currentBlock': 7, 'route': None})()
				
				self.ctc_system.add_train(mock_train_1)
				self.ctc_system.add_train(mock_train_2) 
				self.ctc_system.add_train(mock_train_3)
			except Exception as e:
				pass  # Silently fail if train addition doesn't work
		
		# Use the first train as the error train
		error_train_id = list(self.ctc_system.trains.keys())[0] if self.ctc_system.trains else "1234"
		
		# Initialize emergency state
		self.initialize_emergency_state(error_train_id, "Engine Failure")
		
		# Switch to the emergency tab
		for i in range(self.tab_widget.count()):
			if self.tab_widget.tabText(i) == "Resolve Emergency":
				self.tab_widget.setCurrentIndex(i)
				break

	def initialize_emergency_state(self, error_train_id, error_type="Engine Failure"):
		"""Initialize the emergency state with the train in error and affected trains"""
		...
		if error_train_id in self.ctc_system.trains:
			self.emergency_context['error_train'] = self.ctc_system.trains[error_train_id]
		
		# Find affected trains (trains on same line whose routes may conflict)
		if self.emergency_context['error_train']:
			error_line = self.emergency_context['error_train'].line
			error_section = self.emergency_context['error_train'].sectionLocation
			
			for train_id, train in self.ctc_system.trains.items():
				if train_id != error_train_id and train.line == error_line:
					# Check if train's route passes through error section or nearby sections
					self.emergency_context['affected_trains'].append(train)
		
		self.update_emergency_tables()

	def update_emergency_tables(self):
		"""Update all emergency tables with current data"""
		if not self.emergency_context:
			return
			
		# Update train in error table
		self.train_error_table.setRowCount(1)
		error_train = self.emergency_context['error_train']
		if error_train:
			self.train_error_table.setItem(0, 0, QTableWidgetItem(self.emergency_context['error_type']))
			self.train_error_table.setItem(0, 1, QTableWidgetItem(error_getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))))
			self.train_error_table.setItem(0, 2, QTableWidgetItem(error_train.line))
			self.train_error_table.setItem(0, 3, QTableWidgetItem(error_train.sectionLocation))
			self.train_error_table.setItem(0, 4, QTableWidgetItem(str(self.get_block_number(error_train.currentBlock))))
			self.train_error_table.setItem(0, 5, QTableWidgetItem("F"))  # Placeholder destination section
			self.train_error_table.setItem(0, 6, QTableWidgetItem(str(error_train.destinationBlock or "20")))
			self.train_error_table.setItem(0, 7, QTableWidgetItem("05:30"))  # Placeholder ETA
			self.train_error_table.setItem(0, 8, QTableWidgetItem(str(error_train.speedKmh)))
		
		# Update affected trains table
		affected_trains = self.emergency_context['affected_trains']
		self.affected_trains_table.setRowCount(len(affected_trains))
		for row, train in enumerate(affected_trains):
			self.affected_trains_table.setItem(row, 0, QTableWidgetItem(getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))))
			self.affected_trains_table.setItem(row, 1, QTableWidgetItem(train.line))
			self.affected_trains_table.setItem(row, 2, QTableWidgetItem(train.sectionLocation))
			self.affected_trains_table.setItem(row, 3, QTableWidgetItem(str(self.get_block_number(train.currentBlock))))
			self.affected_trains_table.setItem(row, 4, QTableWidgetItem("E"))  # Placeholder destination section
			self.affected_trains_table.setItem(row, 5, QTableWidgetItem(str(train.destinationBlock or "16")))
			self.affected_trains_table.setItem(row, 6, QTableWidgetItem("5:52"))  # Placeholder ETA
			self.affected_trains_table.setItem(row, 7, QTableWidgetItem(str(train.speedKmh)))
		
		# Update successfully rerouted trains table
		self.success_trains_table.setRowCount(len(self.successfully_rerouted_trains))
		for row, train in enumerate(self.successfully_rerouted_trains):
			self.success_trains_table.setItem(row, 0, QTableWidgetItem(getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))))
			self.success_trains_table.setItem(row, 1, QTableWidgetItem(train.line))
			self.success_trains_table.setItem(row, 2, QTableWidgetItem(train.sectionLocation))
			self.success_trains_table.setItem(row, 3, QTableWidgetItem(str(self.get_block_number(train.currentBlock))))
			self.success_trains_table.setItem(row, 4, QTableWidgetItem("E"))  # Placeholder destination section
			self.success_trains_table.setItem(row, 5, QTableWidgetItem(str(train.destinationBlock or "22")))
			self.success_trains_table.setItem(row, 6, QTableWidgetItem("7:30"))  # Placeholder ETA
			self.success_trains_table.setItem(row, 7, QTableWidgetItem(str(train.speedKmh)))
		
		# Update unable to reroute trains table
		self.unable_trains_table.setRowCount(len(self.unable_to_reroute_trains))
		for row, train in enumerate(self.unable_to_reroute_trains):
			self.unable_trains_table.setItem(row, 0, QTableWidgetItem(getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))))
			self.unable_trains_table.setItem(row, 1, QTableWidgetItem(train.line))
			self.unable_trains_table.setItem(row, 2, QTableWidgetItem(train.sectionLocation))
			self.unable_trains_table.setItem(row, 3, QTableWidgetItem(str(self.get_block_number(train.currentBlock))))
			self.unable_trains_table.setItem(row, 4, QTableWidgetItem("E"))  # Placeholder destination section
			self.unable_trains_table.setItem(row, 5, QTableWidgetItem(str(train.destinationBlock or "10")))
			self.unable_trains_table.setItem(row, 6, QTableWidgetItem("6:22"))  # Placeholder ETA
			self.unable_trains_table.setItem(row, 7, QTableWidgetItem(str(train.speedKmh)))

	def generate_new_routes_for_trains(self, trains):
		"""Placeholder function to generate new routes for affected trains"""
		...
		for train in trains:
			# Simple simulation: 80% success rate for rerouting
			import random
			if random.random() < 0.8:
				# Successfully rerouted
				new_route = [train.currentBlock + i for i in range(1, 6)]  # Placeholder route
				new_routes[getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))] = new_route
				train.route = new_route
				successfully_rerouted.append(train)
			else:
				# Unable to reroute
				unable_to_reroute.append(train)
		
		return {
			'successfully_rerouted': successfully_rerouted,
			'unable_to_reroute': unable_to_reroute,
			'new_routes': new_routes
		}

	def auto_reroute_all_trains(self):
		"""Automatically re-route all affected trains"""
		if not self.emergency_context or not self.emergency_context['affected_trains']:
			StyledMessageBox.warning(self, "No Emergency", "No affected trains to reroute.")
			return
		
		# Generate new routes
		result = self.generate_new_routes_for_trains(self.emergency_context['affected_trains'])
		
		# Update the lists
		self.successfully_rerouted_trains.extend(result['successfully_rerouted'])
		self.unable_to_reroute_trains.extend(result['unable_to_reroute'])
		
		# Remove rerouted trains from affected list
		for train in result['successfully_rerouted'] + result['unable_to_reroute']:
			if train in self.emergency_context['affected_trains']:
				self.emergency_context['affected_trains'].remove(train)
		
		# Update display
		self.update_emergency_tables()
		
		# Show results
		success_count = len(result['successfully_rerouted'])
		failed_count = len(result['unable_to_reroute'])
		StyledMessageBox.information(self, "Rerouting Complete", 
			f"Successfully rerouted {success_count} trains.\n{failed_count} trains require manual intervention.")

	def stop_all_affected_trains(self):
		"""Stop all affected trains by setting authority and speed to 0"""
		if not self.emergency_context or not self.emergency_context['affected_trains']:
			StyledMessageBox.warning(self, "No Emergency", "No affected trains to stop.")
			return
		
		stopped_count = 0
		for train in self.emergency_context['affected_trains']:
			# Set authority and suggested speed to 0
			self.ctc_system.trainAuthorities[getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))] = 0
			self.ctc_system.trainSuggestedSpeeds[getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))] = 0
			train.speedKmh = 0
			stopped_count += 1
		
		StyledMessageBox.information(self, "Trains Stopped", 
			f"Successfully stopped {stopped_count} affected trains.\nAuthority and suggested speeds set to 0.")

	def manually_reroute_selected_train(self):
		"""Navigate to route train tab with selected train pre-loaded"""
		selected_rows = self.unable_trains_table.selectionModel().selectedRows()
		if not selected_rows:
			StyledMessageBox.warning(self, "No Selection", "Please select a train to manually reroute.")
			return
		
		row = selected_rows[0].row()
		if row >= len(self.unable_to_reroute_trains):
			return
			
		selected_train = self.unable_to_reroute_trains[row]
		
		# Switch to Route Train tab
		route_tab_index = None
		for i in range(self.tab_widget.count()):
			if self.tab_widget.tabText(i) == "Route Train":
				route_tab_index = i
				break
		
		if route_tab_index is not None:
			# Set flag to remember we're in emergency context
			self.returning_from_emergency = True
			self.emergency_selected_train = selected_train
			
			# Switch to route train tab
			self.tab_widget.setCurrentIndex(route_tab_index)
			
			# Pre-populate the train in the route train interface
			if hasattr(self, 'current_trains_table'):
				# Find and select the train in the current trains table
				for row in range(self.current_trains_table.rowCount()):
					train_id_item = self.current_trains_table.item(row, 0)
					if train_id_item and train_id_item.text() == selected_getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown'))):
						self.current_trains_table.selectRow(row)
						break

	def stop_selected_train(self):
		"""Stop the selected train from the unable to reroute list"""
		selected_rows = self.unable_trains_table.selectionModel().selectedRows()
		if not selected_rows:
			StyledMessageBox.warning(self, "No Selection", "Please select a train to stop.")
			return
		...
		# Set authority and suggested speed to 0
		self.ctc_system.trainAuthorities[selected_getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))] = 0
		self.ctc_system.trainSuggestedSpeeds[selected_getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))] = 0
		selected_train.speedKmh = 0
		
		StyledMessageBox.information(self, "Train Stopped", 
			f"Train {selected_getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown')))} has been stopped.\nAuthority and suggested speed set to 0.")
		
		# Update the display
		self.update_emergency_tables()

	def finish_resolving_emergency(self):
		"""Show confirmation dialog and finish emergency resolution"""
		if not self.emergency_context:
			return
			
		# Build list of unresolved trains
		unresolved_trains = [getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown'))) for train in self.unable_to_reroute_trains]
		
		# Create confirmation dialog
		msg = StyledMessageBox(self)
		msg.setWindowTitle("Are you sure you are finished?")
		
		dialog_text = ("The block with the train in error will be closed. "
					  "Please use the open/close block interface to reopen it.\n\n")
		
		if unresolved_trains:
			dialog_text += f"The following trains will be halted and not rerouted: {', '.join(unresolved_trains)}\n\n"
			dialog_text += "Are you sure that you do not want to continue rerouting these trains?"
		else:
			dialog_text += "All trains have been successfully handled."
		...
		yes_button.setText("Yes (exit emergency interface)")
		no_button.setText("No (continue rerouting trains)")
		
		result = msg.exec_()
		
		if result == QMessageBox.Yes:
			# Send changes to wayside controller and update system state
			self.send_emergency_resolution_to_wayside()
			
			# Close the block with the error train
			if self.emergency_context['error_train']:
				error_train = self.emergency_context['error_train']
				# Block the section where the error occurred
				if hasattr(self.failure_manager, 'close_block'):
					self.failure_manager.close_block(error_train.line, self.get_block_number(error_train.currentBlock))
			...
			# Clear all tables
			self.train_error_table.setRowCount(0)
			self.affected_trains_table.setRowCount(0)
			self.success_trains_table.setRowCount(0)
			self.unable_trains_table.setRowCount(0)
			# Clear shown conflict IDs so new conflicts can be shown
			if hasattr(self, 'shown_conflict_ids'):
				self.shown_conflict_ids.clear()
			StyledMessageBox.information(self, "Emergency Resolved", 
				"Emergency has been resolved. All changes have been sent to the wayside controller.")

	def send_emergency_resolution_to_wayside(self):
		"""Send route changes and halt instructions to the wayside controller"""
		# Placeholder function for wayside communication
		if not self.emergency_context:
			return
			
		# In a real implementation, this would:
		# 1. Send new routes for successfully rerouted trains
		# 2. Send halt commands for stopped trains
		# 3. Update track status with blocked sections
		# 4. Confirm all changes were applied
		
		print("Sending emergency resolution to wayside controller...")
		print(f"- Successfully rerouted trains: {len(self.successfully_rerouted_trains)}")
		print(f"- Halted trains: {len(self.unable_to_reroute_trains)}")
		if self.emergency_context['error_train']:
			print(f"- Blocked section: {self.emergency_context['error_train'].line} Line, Block {self.get_block_number(self.emergency_context['error_train'].currentBlock)}")

	# Handle return from manual routing
	def handle_train_routed_successfully(self, train_id):
		"""Called when a train is successfully routed from the Route Train tab"""
		if hasattr(self, 'returning_from_emergency') and self.returning_from_emergency:
			# Find the train in unable list and move to success list
			for i, train in enumerate(self.unable_to_reroute_trains):
				if getattr(train, 'id', getattr(train, 'trainID', getattr(train, 'train_id', 'Unknown'))) == train_id:
					# Move to successfully rerouted
					self.successfully_rerouted_trains.append(train)
					self.unable_to_reroute_trains.pop(i)
					
					# Update display
					self.update_emergency_tables()
					
					# Return to emergency tab
					for i in range(self.tab_widget.count()):
						if self.tab_widget.tabText(i) == "Resolve Emergency":
							self.tab_widget.setCurrentIndex(i)
							break
					...
					# Clear return flag
					self.returning_from_emergency = False
					self.emergency_selected_train = None
					break

	# Update methods (using separated business logic)
	def update_data_only(self):
		"""High frequency update - data processing only"""
		pass
...
# Removed compact tables functionality that affects map size
		# QTimer.singleShot(50, self.make_tables_compact)
	...
	def update_train_info_table(self):
		"""Update the train information table using train manager data - show only routed trains"""
		all_train_data = self.ctc_system.get_train_info_for_display()
	...
	def update_current_trains_table(self, filtered_train_data):
		...
		if hasattr(self, 'ctc_system') and self.ctc_system:
			all_train_data = self.ctc_system.get_train_info_for_display()
			print(f"Debug: All trains available: {len(all_train_data)}, Routed trains: {len(filtered_train_data)}")
		else:
			all_train_data = []
	...
	def update_warnings_table(self):
		"""Update the warnings table using maintenance manager"""
		warnings = self.failure_manager.generate_warnings(
			self.ctc_system.trackStatus,
			self.ctc_system.railwayCrossings
		)
	...
	def detect_and_handle_conflicts(self):
		"""Detect conflicts using the collision detector and handle critical ones"""
		try:
			# Detect all conflicts
			conflicts = self.ctc_system.detect_conflicts()
			
			# Check for critical conflicts that need immediate attention
			critical_conflicts = [c for c in conflicts if c.severity.value <= 2]  # Critical or High
			
			if critical_conflicts:
				# Show warning for critical conflicts
				for conflict in critical_conflicts:
					if conflict.is_collision_imminent():
						# Immediate collision risk - take emergency action
						self.handle_emergency_conflict(conflict)
					else:
						# Log the conflict for dispatcher attention
						self.log_conflict_warning(conflict)
						
		except Exception as e:
			# Don't let conflict detection errors crash the UI
			print(f"Error in conflict detection: {e}")
	
	def handle_emergency_conflict(self, conflict):
		"""Handle emergency conflicts requiring immediate intervention"""
		# Only show popup if this conflict hasn't been shown yet
		if hasattr(self, 'shown_conflict_ids') and conflict.conflict_id in self.shown_conflict_ids:
			return
		if hasattr(self, 'shown_conflict_ids'):
			self.shown_conflict_ids.add(conflict.conflict_id)
		# Create urgent warning message
		warning_msg = f"""EMERGENCY CONFLICT DETECTED!
		
Conflict Type: {conflict.conflict_type.value.replace('_', ' ').title()}
Severity: {conflict.severity.name}
Trains Involved: {', '.join(conflict.train_ids)}
Location: {conflict.location_line} Line, Block {conflict.location_block}
Time to Collision: {conflict.estimated_time_to_collision:.1f} seconds

Immediate Actions Required:
{chr(10).join([f"• {action}" for action in conflict.suggested_actions])}
"""
		# Show critical warning dialog
		msg_box = StyledMessageBox(self)
		msg_box.setWindowTitle("EMERGENCY CONFLICT ALERT")
		...
		""")
		msg_box.exec_()
		# Mark conflict as acknowledged (resolved will be done manually)
		self.ctc_system.resolve_conflict(conflict.conflict_id, "Emergency alert shown to dispatcher")
	
	def log_conflict_warning(self, conflict):
		"""Log non-critical conflicts for dispatcher awareness"""
		# For now, just print to console
		# In the future, this could add to a conflict log table
		print(f"CONFLICT WARNING: {conflict.conflict_type.value} - Trains {', '.join(conflict.train_ids)} at {conflict.location_line} Line Block {conflict.location_block}")
	...
	def update_block_info_table(self):
		...
		for row in range(self.block_info_table.rowCount()):
			...
			if line_item and block_item:
				...
				# Use state manager as primary source of truth
				is_closed = self.display_manager.is_block_closed(line, block_num) if hasattr(self.display_manager, 'is_block_closed') else False
				# Fallback to maintenance manager if needed
				if not is_closed and hasattr(self.failure_manager, 'is_block_closed'):
					is_closed = self.failure_manager.is_block_closed(line, block_num)
				open_text = "○" if is_closed else "●"  # ● = filled circle (open), ○ = empty circle (closed)
				if open_item:
					if open_item.text() != open_text:
						open_item.setText(open_text)
						print(f"DEBUG: Updated block {line} {block_num} status to {'CLOSED' if is_closed else 'OPEN'}")
				else:
	...
	def check_route_possibility(self, line, section, block):
		...
		try:
			...
			if route is not None and route.total_time < float('inf'):
				# Route is possible
				return True, section, block
			else:
				...
				for test_block in test_blocks:
					...
					if test_route is not None and test_route.total_time < float('inf'):
		...
		try:
			...
			if system_time_str:
				try:
					...
					current_time = datetime.now().replace(hour=current_hour, minute=current_minute, second=0, microsecond=0)
				except:
					current_time = datetime.now()  # Fallback if system time parsing fails
			else:
				current_time = datetime.now()
	...
	def calculate_arrival_from_departure(self, departure_time):
		...
		try:
			...
			if route and route.total_time < float('inf'):
	...
	def check_time_possibility(self, requested_time):
		...
		try:
			...
			if route is None or route.total_time == float('inf'):
				# Route not possible at all
				return False, requested_time
			
			# Calculate the earliest possible arrival time
			current_time = datetime.now()
			earliest_arrival = current_time.timestamp() + route.total_time
		...
		try:
			...
			if not success:
				if isinstance(routes_or_error, str) and "syntax" in routes_or_error.lower():
					...
					# Routes not possible
					failed_trains = ["2345", "3456"]  # Example failed trains
					revised_routes = "Train 2345 destination changed to block 20.\nTrain 3456 arrival time changed to 5:30."
	...
	def parse_schedule_csv(self, file_path):
		"""Placeholder CSV parsing function"""
		# For now, return sample data
		sample_routes = [
			{"train_id": "2345", "on_track": "○", "moving": "○", "line": "Green", 
			 "location_section": "Yard", "location_block": "Yard", "dest_section": "I", "dest_block": "37", "eta": "5:30"},
			{"train_id": "3456", "on_track": "○", "moving": "○", "line": "Red", 
			 "location_section": "Yard", "location_block": "Yard", "dest_section": "I", "dest_block": "47", "eta": "6:30"}
		]
		return True, sample_routes
	...
	def implement_route(self):
		...
		try:
			...
			if success:
				...
				# If we're returning from emergency, notify the emergency handler
				if hasattr(self, 'returning_from_emergency') and self.returning_from_emergency:
					self.handle_train_routed_successfully(train_id)
			else:
	...
	def generate_and_display_route(self):
		...
		try:
			...
			if arrival_time:
				try:
					...
					if system_time_str:
						try:
							current_hour, current_minute = map(int, system_time_str.split(':'))
							current_time_base = datetime.now().replace(hour=current_hour, minute=current_minute)
					...
					if system_time_str:
						try:
							current_hour, current_minute = map(int, system_time_str.split(':'))
							current_check_time = datetime.now().replace(hour=current_hour, minute=current_minute, second=0, microsecond=0)
						except:
							pass  # use real time
			...
			# For route calculation, we need to handle existing vs new trains differently
			if train_id in self.ctc_system.trains:
				# Existing train - calculate route normally
				route = self.ctc_system.calculate_route(
					train_id, destination_block, 'SAFEST_PATH',
					scheduled_arrival=scheduled_arrival
				)
			else:
				# New train - create a temporary train for route calculation only (doesn't affect ID counters)
				temp_start_block = 1  # Default yard location
				temp_train_id = self.ctc_system.add_temporary_train(line, temp_start_block, train_id)
				route = self.ctc_system.calculate_route(
					temp_train_id, destination_block, 'SAFEST_PATH',
					scheduled_arrival=scheduled_arrival
				)
				# Remove the temporary train immediately
				self.ctc_system.remove_train(temp_train_id)
			
			if route is None or route.total_time == float('inf'):
				StyledMessageBox.warning(self, "Route Error", 
					"Unable to calculate a safe route to the destination. Please check for conflicts or maintenance closures.")
				return
	...
	def generate_route_data_for_proposed_route(self, train_id, line, destination_block, arrival_time, route):
		...
		if arrival_time and arrival_time.strip():
		...
		elif route and route.total_time < float('inf'):
			...
			if current_time:
				try:
					...
					current_datetime = datetime.now().replace(hour=current_hour, minute=current_minute, second=0, microsecond=0)
				except:
					current_datetime = datetime.now()
			else:
				current_datetime = datetime.now()
			eta_datetime = current_datetime + timedelta(seconds=route.total_time)
		...
		if display_eta and display_eta != "Blocked":
			try:
				...
				if route and route.total_time < float('inf'):
					travel_minutes = int(route.total_time / 60)
	...
	def generate_placeholder_route(self, train_id, line, section, block, arrival_time):
		"""Generate a placeholder route for display"""
		...
		# Calculate departure time based on arrival time and estimated travel duration
	...
	def calculate_precise_travel_time(self, train_id, destination_block):
		"""Calculate precise travel time using speed limits and door times (60s per stop)"""
		try:
			# Calculate route using route manager
			
			# Get the train's line and starting position
			if train_id in self.ctc_system.trains:
				train = self.ctc_system.trains[train_id]
				line = train.line
				start_block = self.get_block_number(train.currentBlock)
			else:
				# For new trains, get line from train ID and assume yard start
				line = self.ctc_system.id_manager.get_line_from_train_id(train_id)
				start_block = 1  # Yard position
			
			# Calculate route to get the path
			if train_id in self.ctc_system.trains:
				route = self.ctc_system.calculate_route(train_id, destination_block, 'SAFEST_PATH')
			else:
				# Create temporary train for route calculation
				temp_train_id = self.ctc_system.add_temporary_train(line, start_block, train_id)
				route = self.ctc_system.calculate_route(temp_train_id, destination_block, 'SAFEST_PATH')
				self.ctc_system.remove_train(temp_train_id)
			
			if not route or route.total_time == float('inf'):
				return 0  # Route not possible
			
			# Calculate precise timing using speed limits and door times
			total_time_seconds = 0
			blocks_in_route = route.get_block_numbers()
			
			for i, block_num in enumerate(blocks_in_route):
				block_info = self.trackReader.get_block_info(line, block_num)
				if not block_info:
					continue
				
				# Calculate travel time for this block
				speed_limit_kmh = block_info.speed_limit_kmh
				block_length_m = block_info.length_m
				
				# Convert speed from km/h to m/s
				speed_ms = speed_limit_kmh * 1000 / 3600
				
				if speed_ms > 0:
					travel_time = block_length_m / speed_ms
				else:
					travel_time = 60  # Default if no speed data
				
				total_time_seconds += travel_time
				
				# Add door time (60 seconds) for station stops
				# Consider a block a station if it has infrastructure or is a known stop
				if self.is_station_block(line, block_num):
					total_time_seconds += 60  # 60 seconds for doors
			
			return total_time_seconds
			
		except Exception as e:
			print(f"Error calculating precise travel time: {e}")
			return 0
	...
	def dispatch_train(self):
		...
		# Plan route using the imported track display
		from ctc import CTCTrackDisplay
		track_display = CTCTrackDisplay(self.trackReader.file_path)
	...
	def resize_panels_for_tables(self):
		"""DEPRECATED: This method forced table sizing that made the map too small"""
		# This functionality has been removed per user request
		# The map was being made too small to eliminate scrolling on block table
		pass
	...
	def make_tables_compact(self):
		"""DEPRECATED: This method forced compact sizing that made the map too small"""
		# This functionality has been removed per user request
		# It was interfering with proper table display and making the map too small
		pass
	...
	def execute_auto_population(self):
		...
		print(f"DEBUG: execute_auto_population called, _auto_populating={getattr(self, '_auto_populating', 'not set')}")
		if hasattr(self, '_auto_populate_data') and self._auto_populate_data:
			...
			print(f"DEBUG: Auto-populating maintenance tab with line={line}, block={block}")
			self.populate_maintenance_tab(line, block)
			# Clear the data
			self._auto_populate_data = None
		else:
			print("DEBUG: No auto-populate data found")
		# Reset the flag after population
		self._auto_populating = False
		print("DEBUG: Auto-population complete, flag reset")

	def populate_maintenance_tab(self, line: str, block: int):
		"""Populate the maintenance tab with selected block info"""
		print(f"DEBUG: populate_maintenance_tab called with line={line}, block={block}")
	...
	def on_trains_updated(self):
		...
		print("DEBUG: on_trains_updated() called - train state has changed")
	...
	def on_warnings_updated(self):
		...
		print("DEBUG: on_warnings_updated() called - warnings have changed")
		# Update warnings table immediately
		self.update_warnings_table()

	def on_maintenance_updated(self):
		"""Handle maintenance closure state changes"""
		print("DEBUG: on_maintenance_updated() called - maintenance state has changed")
	...
	def update_warnings_display(self):
		"""Update warnings display from state manager"""
	...
	def update_scheduled_closures_display(self):
		...
		if hasattr(self, 'scheduled_closures_table'):
			scheduled = self.failure_manager.get_scheduled_closures()
			print(f"DEBUG: update_scheduled_closures_display() - Total scheduled closures: {len(scheduled)}")
			for s in scheduled:
				block_id = s.get('block_id', s.get('block_number', 'Unknown'))
				line = s.get('line', 'Unknown')
				status = s.get('status', 'Unknown')
				print(f"  - Block {block_id} on {line}: status={status}")
			...
			print(f"DEBUG: Active/scheduled closures to display: {len(active_scheduled)}")
	...
	def on_tab_changed(self, index):
		...
		if not self._auto_populating:
			...
			elif index == 3:  # Resolve Emergency tab
				self.reset_emergency_tab()
	...
	def reset_emergency_tab(self):
		"""Reset the emergency tab to its default state"""
		# Clear any emergency context
		if hasattr(self, 'emergency_context'):
			self.emergency_context = None
		
		# Clear emergency-related flags
		if hasattr(self, 'returning_from_emergency'):
			self.returning_from_emergency = False
		if hasattr(self, 'emergency_selected_train'):
			self.emergency_selected_train = None
