...
from train_controller_sw.controller.data_types import (TrainModelInput, DriverInput, EngineerInput, 
...
track_reader_path = os.path.join(os.path.dirname(__file__), '..', '..', 'Track_Reader')
sys.path.append(track_reader_path)
...
class TrainController:
    ...
    def __init__(self, init_data: TrainControllerInit, kp: float = 12, ki: float = 1.2):
        ...
        required_fields = ['track_color', 'current_block', 'current_commanded_speed', 'authorized_current_block', 'next_four_blocks']
        missing_fields = []
        ...
        try:
            excel_path = os.path.join(track_reader_path, "Track Layout & Vehicle Data vF2.xlsx")
    ...
    def update(self, train_input: TrainModelInput, driver_input: DriverInput) -> None:
        """
        Update controller output using real-time PI control and driver inputs.
        ...
        # Update train ID from driver input
        self.current_output.train_id = driver_input.train_id
        ...
        if self.auto_mode:
            ...
            # Check for faults and activate emergency brake if needed
            fault_detected = (train_input.fault_status.get('signal', False) or 
                            train_input.fault_status.get('brake', False) or 
                            train_input.fault_status.get('engine', False))
        ...
        # 3. In auto mode AND faults are detected
        if driver_input.emergency_brake or train_input.passenger_emergency_brake or (self.auto_mode and fault_detected):
            self.current_output.emergency_brake_status = True
            self.current_output.power_kw = 0.0
            self.integral_error = 0.0
        else:
    ...
    def calculate_power(self, train_input: TrainModelInput, dt: float) -> float:
        """
        Real-time PI controller for motor power calculation.
